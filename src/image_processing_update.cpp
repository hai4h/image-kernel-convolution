...
int cyclicIndex(int index, int limit) {
    return (index + limit) % limit;
}
double calculateMean(const cv:: Mat& image,int x, int y){
    double sum = 0.0;
    for(int i = -1 ; i < 2 ; i++){
        for (int j = -1 ; j < 2 ; j++){
            int kx =cyclicIndex(x + i , image.rows);
            int ky = cyclicIndex(y + j , image.cols);
            sum += image.at<uchar>(kx,ky);
        }
    }
    return sum / 9.0;
}
double calculateVaricance(const cv::Mat& image , int x , int y){
    double mean = calculateMean(image , x,y);
    double variance = 0.0;
    for(int i = -1 ; i < 2 ; i++){
        for (int j = -1 ; j < 2 ; j++){
            int kx =cyclicIndex(x + i , image.rows);
            int ky = cyclicIndex(y + j , image.cols);
            variance += (image.at<uchar>(kx,ky) - mean) * (image.at<uchar>(kx,ky) - mean) ;
        }
    }
    return variance /9.0;
}
void applyStatisticalFilter(const cv::Mat& inputImage , cv::Mat& outputImage){
    ...
    for (int i = 0 ; i < rows; i++){
        for(int j = 0 ; j < cols ; j++){
            double mean = calculateMean(inputImage , i,j);
            double variance = calculateVaricance(inputImage , i , j);
            if (abs(inputImage.at<uchar>(i , j) - mean ) < M * variance) {
                outputImage.at<uchar>(i,j) = inputImage.at<uchar>(i,j);
            }else{
                outputImage.at<uchar>(i,j) = clamp(mean);
            }
        }
    }
}
